...
4. 執行完畢後下方「執行記錄」應該看到 6 行報告，5 項全綠勾：
   ```
   === selfCheck (pet-grooming) ===
   ✓ 容器檔案 A（客戶資料）：N 列
   ✓ 容器檔案 A（服務紀錄）：M 列｜散客結單目標
   ✓ 檔案 B（交易明細）：K 列｜儲值結單目標
   ✓ 檔案 B（包月客戶）：J 列｜包月中 P 位｜結單目標
   ✓ LINE API 連通性：HTTP 401（網路通）
   ```
   > 沒開包月分頁也沒關係：「✓ 檔案 B（包月客戶）：分頁不存在（包月功能未啟用，正常）」也算過。
   > 5/12 起新增「服務紀錄」分頁檢查（散客結單寫這裡）— 沒有就「✗ 找不到分頁：服務紀錄」需要先在檔案 A 開分頁。
5. 哪一項打 ✗ 就照細節修：
   - **容器檔案 A（客戶資料）** ✗ `找不到分頁：客戶資料` → 工作表名稱不對，回檔案 A 改名或改 Code.gs 第一行 `SHEET_NAME`
   - **容器檔案 A（服務紀錄）** ✗ `找不到分頁：服務紀錄` → 檔案 A 還沒有「服務紀錄」分頁。在檔案 A 加新分頁，名稱叫「服務紀錄」，第一列建 8 欄 header：`日期 / 主人電話 / 寵物名 / 服務項目 / 金額 / 付款方式 / 美容師 / 備註`（散客結單會寫這裡）
...
## 🆕 5/12 升級：員工 LIFF 加「結單」功能（doPost 寫入）

5/12 起 Code.gs 加了 `doPost(e)`：員工 LIFF 多了「📝 結單」tab，可以直接在平板上結單寫進 Sheets，淘汰紙本。

**首次升級流程**（已有部署的人）：
1. 把新版 `Code.gs` 整份重貼進 Apps Script 編輯器（覆蓋舊版）
2. **跑一次 `selfCheck`**：確認多了一項「容器檔案 A（服務紀錄）」是否綠勾
   - 如果 ✗ `找不到分頁：服務紀錄` → 在檔案 A 開新分頁「服務紀錄」+ 8 欄 header（見步驟 3 疑難排解）
3. **重新部署新版本**（**必做**，否則 LIFF 端 POST 會打到舊版 doPost 不存在的部署）：
   - 部署 → 管理部署 → 齒輪 → 編輯
   - 版本下拉選「**新版本**」（不是「目前版本」）
   - 說明可以寫「v2 加結單」
   - 部署
   - URL **不變**，所以 customer.html 不用改
4. 手機開 LIFF → 切到「📝 結單」tab → 隨便結一筆散客（寵物名 + 100 元）→ 看檔案 A「服務紀錄」分頁有沒有寫進去
5. 切到「儲值」mode 搜一個有餘額的客戶 → 結 1 元測試 → 看交易明細有沒有正確扣 1 元

**沒重新部署的症狀**：LIFF 結單按下「送出」→ 跳「HTTP 302」或「Unexpected token <」error。原因是部署的還是舊版本（沒有 doPost handler）。

## 📈 後續可以加的功能（V3+）

- **加存取密碼**：Code.gs 檢查 `e.parameter.key === "XXX"`，index.html 加 query string
- **結單可選美容師**：目前共用平板會把所有單記在同一個帳號名下，可在散客 mode 加美容師選擇
