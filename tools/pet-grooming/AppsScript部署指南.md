...
4. 執行完畢後下方「執行記錄」應該看到 5 行報告，4 項全綠勾：
   ```
   === selfCheck (pet-grooming) ===
   ✓ 容器檔案 A（客戶資料）：N 列
   ✓ 檔案 B（交易明細）：M 列
   ✓ 檔案 B（包月客戶）：K 列｜包月中 P 位
   ✓ LINE API 連通性：HTTP 401（網路通）
   ```
   > 沒開包月分頁也沒關係：「✓ 檔案 B（包月客戶）：分頁不存在（包月功能未啟用，正常）」也算過。
5. 哪一項打 ✗ 就照細節修：
   - **容器檔案 A** ✗ `找不到分頁：客戶資料` → 工作表名稱不對，回檔案 A 改名或改 Code.gs 第一行 `SHEET_NAME`
   - **檔案 B（交易明細 / 包月客戶）** ✗ `You do not have permission...` → 老闆帳號沒被加進「寵物店儲值帳本」共用清單，或老闆娘把帳號移除了 → 補回共用清單再跑一次
   - **LINE API 連通性** ✗ `HTTP 5xx` 或網路錯 → Google 端問題，重試
6. 全綠後可以再選 `testReadCustomers` 看 client 拿到的客戶 JSON 長相，或選 `testMonthlyMap` 單獨測包月狀態
