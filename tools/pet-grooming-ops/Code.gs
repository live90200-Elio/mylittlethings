...
function buildGuideRows_() {
  ...
  return [
    ...
    { kind: "kv", a: "每天主動：建明日預約",
      b: "Google 日曆建事件：\n  • 標題：寵物名 + 服務記號（剪毛加 ✂️ 或「剪」字，例：小白✂️）\n  • 描述欄：客戶手機（格式不限：0912-345-678 / 0912345678 / +886-912-345-678 都會自動辨識）\n  • 店休：標題寫「特休 / 公休 / 休假 / 店休」會自動跳過" },
    { kind: "kv", a: "不定期主動：補客戶資料",
      b: "檔案 A「客戶資料」分頁：\n  • G 欄「重要提醒」（兇 / 心臟病 / 怕剪刀聲；分號分隔）\n  • I 欄「最近到店」\n（這 2 欄 LIFF 不會自動寫，靠你手動補）\n\n💡 儲值/餘額**不在這張表填**，統一進「寵物店儲值帳本」（檔案 B）「交易明細」分頁，員工查詢頁 + 客戶 LIFF 都從那邊算。" },
    { kind: "kv", a: "不定期主動：填儲值客戶交易（範例）",
      b: "📍 為什麼一定要填這裡：客戶在 LINE 看的「儲值金餘額」+「最近 5 筆」全部從檔案 B「交易明細」即時算。沒填 = 客戶看到餘額 0、什麼紀錄都沒有。\n\n📋 範例：客戶買「3000+400」+ 當天消費 600 → 加 2 列\n  列1：電話 / 姓名 / 2026/05/03 / D=3000+400 / E空 / F空 / G=3400 / H空 / I=5/3 儲值\n  列2：電話 / 姓名 / 2026/05/03 / D空 / E=洗澡/藥浴/基礎美容 / F=600 / G=2800 / H空 / I=5/3 消費\n\n📋 純消費（不買新儲值）→ 1 列：D空、E+F+G 照填\n📋 純買儲值（沒消費）→ 1 列：D 填、E+F空、G 填新餘額\n\n⚠️ 三大雷：\n  1️⃣ G 欄「餘額」一定要填（程式取最後一筆有餘額的列當目前餘額）\n  2️⃣ G 欄餘額自己算：上次餘額 + 本次儲值 - 本次消費\n  3️⃣ D 欄寫「3400」或「3000+400」都行（程式會算 + 號）\n\n填完請客戶從 LINE 豐富選單重開儲值選單（不是切回去）就會看到。" },
    { kind: "kv", a: "收班主動：拍照傳 Cowork",
      b: "下班前最後一件事：\n  • 確認當天兩張手寫單員工都填完 + 客戶都簽收\n  • 兩張單分開拍清楚（不折角、不反光、頁首日期要入鏡）\n  • 用 LINE / Drive 傳給 Cowork\n  • 紙本歸檔到月份夾（保留 1-2 年當客戶簽收證據）" },
    { kind: "rule",
      text: "LINE 必須加自己的官方帳號為好友（不加 Push 推不到）\n檔案 A 永遠建在老闆帳號，不要建在員工帳號（員工離職你會失去資料）\n「服務紀錄」E 欄（金額）絕對不能放 =SUM()（員工會看到月營收，違反設計）\n想看月營收 → 看老闆儀表板（PWA）或檔案 B，不是檔案 A\n客戶資料 J-N 欄（建立時間 / 來源 / 本次服務 / 本次金額 / LINE_userId）是 LIFF 自動寫的，別動\n檔案 A、檔案 B 的「共用」清單裡 Elio（工程師）帳號不要移除（員工查詢 LIFF 是用工程師帳號讀檔案 B，被踢出共用 → LIFF 全店爆「Failed to fetch」；5/3 踩過）" },
    { kind: "blank" },
    ...
    { kind: "kv", a: "客人來店流程",
      b: "1️⃣ 員工查詢頁 → 輸入主人手機號 → 看客戶 / 寵物 / 重要提醒 / 美容備註 / 儲值餘額（自動從檔案 B 算）/ 最近到店\n2️⃣ 服務完成 → 客戶屬於哪類？\n   ▸ 一般 / 散客（含 LIFF 簽過約的回頭客） → 寫散客服務簽收單 或 LIFF 結單\n   ▸ 儲值客戶（有儲值金的） → 寫儲值客戶服務簽收單 或 LIFF 結單\n   ▸ 包月客戶 → 寫包月客戶簽收單 或 LIFF 結單\n3️⃣ 客戶在「簽收」欄手寫簽名（紙本路徑）\n4️⃣ 結束！紙本當天放現場，老闆娘下班會收走拍照給 Cowork KEY IN" },
    { kind: "kv", a: "🆕 5/12 起：LIFF 結單（直接平板結，不用紙本）",
      b: "員工 LIFF 多了「📝 結單」tab，按一按直接寫進 Sheets，繞過 Cowork KEY IN：\n\n切到「📝 結單」tab → 選類別（散客 / 儲值 / 包月）：\n  ▸ 散客：寵物名 + 金額 + 付款方式 → 送出（寫服務紀錄）\n  ▸ 儲值：搜寵物名或電話末 4 碼 → 選客戶 → 看餘額 → 輸消費金額 → 看扣後餘額預覽 → 送出（寫交易明細，餘額自動扣）\n  ▸ 包月：搜寵物名或電話末 4 碼 → 選客戶 → 看狀態 → 「扣次數」（自動算第幾次）或「新購」（45 天 5 次，預設 1500）→ 送出（寫包月分頁，已用/剩/到期日自動推算）\n\nLIFF 結的單在 Sheets 備註欄會標「LIFF 結單 by 員工X」方便老闆娘區分\n\n兩條路都可走：客戶在等狗、員工有空 → LIFF 結；員工手忙、客戶催 → 寫紙本最快" },
    { kind: "rule",
      text: "每張手寫單頁首先填一次：日期 + 美容師頁首寫一次就好，每列不要重寫\n電話寫滿 10 碼開頭 0 不能省（漏 0 救不回；4/28 才剛踩過）\n金額純數字（800，不是 NT$800、不是 800元、不是 1,500）\n勾選用 ✓ 或塗滿，不要畫斜線（OCR 容易判錯）\n「客戶資料」分頁只能改 G 欄（重要提醒）、H 欄（美容備註），其他欄位不要動\n⛔ 不要再直接寫進「服務紀錄」分頁（流程已改：寫手寫單 → Cowork KEY IN）" },
    { kind: "kv", a: "客人說來剪過但找不到", b: "可能 LIFF 留錯電話 → 跟老闆反應，老闆來修" },
    { kind: "kv", a: "客人說有用 LIFF 填過表單", b: "客戶資料分頁應該已自動有他了，不用手動建" },
    { kind: "blank" },

    // ===== Cowork =====
    { kind: "h2", text: "📸 Cowork（KEY IN 識別作業）" },
    { kind: "kv", a: "判斷哪張單",
      b: "標題「🐾 洗毛這件小事 — 散客服務簽收單」 → 檔案 A「服務紀錄」分頁\n標題「🐾 洗毛這件小事 — 儲值客戶服務簽收單」 → 檔案 B「交易明細」分頁\n兩張單目標分頁完全不同，不要混在一起 KEY IN" },
    { kind: "kv", a: "頁首套用全列",
      b: "日期：YYYY 年 MM 月 DD 日 → 寫成 YYYY/MM/DD，套該張單全部列\n美容師：XXX → 寫進每列「美容師」欄（僅散客單 G 欄；儲值單沒此欄）" },
    { kind: "kv", a: "散客單欄位 mapping → 服務紀錄",
      b: "(頁首)日期 → A 日期（Date 物件）\n主人電話 → B 主人電話（10 碼字串開頭 0）\n寵物名 → C 寵物名\n服務項目(勾) → D 服務項目（多選用「、」串）\n金額 → E 金額（純數字 integer）\n付款方式(勾) → F 付款方式（單選文字）\n(頁首)美容師 → G 美容師\n備註 → H 備註\n簽收 → 不寫進 Sheets（紙本歸檔當證據）" },
    { kind: "kv", a: "儲值單欄位 mapping → 交易明細",
      b: "主人電話 → A 電話\n寵物名 → B 寵物名\n(頁首)日期 → C 日期\n儲值金額 → D（多空白；只在新增儲值才填）\n服務項目(勾) → E 消費項目\n消費金額 → F 消費金額\n扣後餘額 → G 餘額（員工已算好直接抄）\n簽收 → H 簽名（寫「已簽」即可）\n備註 → I 備註" },
    { kind: "rule",
      text: "電話 10 碼開頭 0 必留：手寫破折號 / 空格清乾淨成純 10 碼字串\nA 欄電話 cell 寫之前先 setNumberFormat(\"@\")，否則 Sheets 自動格式吃掉前導 0（4/28 踩過）\n金額純數字 integer：NT$1,500元 → 1500，不要小數 / 逗號 / 貨幣符號\n日期是 Date 物件不是純文字（營收 trigger 用 instanceof Date 判斷，純文字會被跳過）\n空列跳過：只有 # + ☐ 沒勾沒填的整列不寫\n# 序號不寫進 Sheets（只是行序提示）\n勾選 ✓ / ■ / 點 / 一槓 / 圈圈 → 都視為勾選；只有空白 ☐ 才是沒勾" },
    { kind: "kv", a: "完整版規則 + 欄位順序圖",
      b: "tools/pet-grooming-receipt-form/KEY_IN_RULES.md" },
    { kind: "blank" },
    ...
    { kind: "kv", a: "手寫單列印 + 員工 SOP + 老闆收班流程", b: "tools/pet-grooming-receipt-form/README.md" },
    { kind: "kv", a: "Cowork 完整 KEY IN 規則", b: "tools/pet-grooming-receipt-form/KEY_IN_RULES.md" },
  ];
}
